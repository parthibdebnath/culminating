...
let URL = "https://teachablemachine.withgoogle.com/models/T42wbvZhq/";
...
function checkPose(prediction, video) {
    ...
    if (prob >= 0.75 && !explosionActive) {
        ...
        switch(poseNumber) {
            case '1':
                if (time >= 0 && time <= 7 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '2':
                if (time >= 7 && time <= 8.9 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
              case '3':
                ...
                break;
            case '4':
                if (time >= 11 && time <= 14 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '5':
                if (time >= 14 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
        }
